        ...
        <div className="p-5 border-b border-gray-100 bg-white flex gap-4">
          <div className="relative flex-grow max-w-md">
            ...
            <input 
              ...
              className="pl-11 pr-4 py-3 w-full bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all text-sm font-medium"
            />
          </div>
        </div>

        {loading ? (
          <div className="p-12 flex justify-center"><div className="w-10 h-10 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin"></div></div>
        ) : products.length === 0 ? (
          <div className="p-16 text-center text-gray-500 flex flex-col items-center">
            <div className="bg-indigo-50 p-6 rounded-full mb-6">
              <Package size={56} className="text-indigo-400" strokeWidth={1.5} />
            </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {products.map((product) => (
                  <tr key={product.id} className="hover:bg-gray-50/80 transition-colors group">
                    <td className="p-5 text-sm font-mono font-medium text-gray-500 bg-gray-50/30">
                      {product.sku}
                    </td>
                    <td className="p-5">
                      <div className="font-bold text-gray-900 mb-1">{product.name}</div>
                      <div className="text-xs text-gray-500 truncate max-w-xs">{product.description || 'No description provided'}</div>
                    </td>
                    <td className="p-5 font-black text-gray-900">${product.price.toFixed(2)}</td>
                    <td className="p-5">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-5">
                <div className="col-span-2">
                  ...
                  <input required type="text" name="name" value={formData.name} onChange={handleChange} className="w-full bg-gray-50 border-none rounded-xl p-3 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all" placeholder="e.g. Wireless Headphones" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input required type="number" min="0" name="stock" value={formData.stock} onChange={handleChange} className="w-full bg-gray-50 border-none rounded-xl p-3 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all" placeholder="0" />
                </div>
                <div className="col-span-2">
                  <label className="block text-sm font-bold text-gray-700 mb-2">Unit Price ($)</label>
                  <div className="relative">
                    <span className="absolute left-4 top-3 text-gray-500 font-bold">$</span>
                    <input required type="number" min="0.01" step="0.01" name="price" value={formData.price} onChange={handleChange} className="w-full bg-gray-50 border-none rounded-xl py-3 pl-8 pr-4 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all font-medium" placeholder="99.99" />
                  </div>
                </div>
                <div className="col-span-2">
                  ...
                  <textarea name="description" value={formData.description} onChange={handleChange} rows="3" className="w-full bg-gray-50 border-none rounded-xl p-3 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all resize-none" placeholder="Brief product description..."></textarea>
                </div>
              </div>
